...
export async function obtenerTodosLosPersonales() {
    const [rows] = await pool.query(`
        SELECT p.*, u.estado_usuario, u.nombre_usuario 
        FROM personal p
...
export async function crearPersonal(datos) {
    const { nombre_per, apellido_per, dni_per, telefono_per, correo_elect_per } = datos;
    ...
    try {
        ...
        await connection.query(
            ...
            [nombreUsuario, contraseniaHash, 'inactivo', 1, idPersonal]
        )
